...
brand = "All"
zone_options = [
    "West", "MP&Rajasthan", "North", "Gujarat", "South", "East"
]

# ZONAL HEAD
if role == "Zonal Head":
    zone = st.selectbox("Zone Access", zone_options)
    brand = "All"
    st.text_input("Brand Access", value="All", disabled=True)
...
# BRAND MANAGER
    ...
    selected_zone = st.selectbox(
        "Zone", zone_options, key="selected_zone"
    )
    selected_brand = st.selectbox(
        "Brand", brand_options, key="selected_brand"
    )
    
    if st.button("➕ Add Access"):
        access_pair = f"{selected_zone}|{selected_brand}"
        if access_pair not in st.session_state.access_mapping_list:
            st.session_state.access_mapping_list.append(access_pair)
            st.rerun()
    
    st.markdown("### Current Access")
    for idx, access in enumerate(st.session_state.access_mapping_list):
...
# FULL ACCESS ROLES
    ...
    st.info("This role will have access to All Zones and All Brands.")

# ==================================
# ADD USER BUTTON
# ==================================
access_mapping = ";".join(st.session_state.access_mapping_list)

if st.button("➕ Add User", use_container_width=False):
    if not user_id or not password or not name:
        ...
        st.error("Please add at least one Zone-Brand Access.")
    else:
        success = add_user(
            user_id,
            password,
